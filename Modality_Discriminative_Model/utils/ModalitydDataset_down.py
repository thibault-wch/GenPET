...
class ModalitydDataset(torch.utils.data.Dataset):
    def __init__(self, mode="train", tasktype="all"):
        ...
        self.transform1 = mtransforms.OneOf(
            [
                mtransforms.Resized(keys=["mri", "pet"], spatial_size=(96, 112, 96)),
                mtransforms.RandSpatialCropd(keys=["mri", "pet"], roi_size=(96, 112, 96))
            ]
            , weights=(0.5, 0.5)
        )
    ...
    def __getitem__(self, index):
        if self.mode=='train':
            ...
            item =self.transform1(self.basic_transform(
                    self.imgs[index]
                ))
